...
def Image_steganography(file, n,key):
    ...
    def Encode():
        ...
        w, h = img.size
        ciph = input("[*] Enter the secret message:- ")
        data = security.encrypt_text(key,ciph)
        if len(data) == 0:
    ...
    def Decode():
        ...
        image = cv2.imread(file)
        img = Image.open(file, 'r')
        deciph = find_data(image)
        img.close()
        msg=security.decrypt_text(key,deciph)
        print("[*] The Encoded data was: \u001b[38;5;129m{}".format(msg))
